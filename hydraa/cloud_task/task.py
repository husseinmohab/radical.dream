...
class Task(Future):
    ...
    Attributes:
        ...
        vcpus (float): Number of virtual CPUs to allocate. 
        memory (float): Amount of memory in GB to allocate.
        run_id (str): Identifier for the current run.
        provider (str): Name of cloud provider.
        args (List[str]): List of command line arguments.
        ip (Optional[str]): IP address of the instance running this task.
        cmd (Optional[str]): Command to execute for the task.
        depends_on (List[str]): List of task IDs this task depends on.
        arn (Optional[str]): ARN (AWS only) identifier for the task instance.
        type (Optional[str]): Type of task (e.g. paramiko, docker).
        port (Optional[int]): Port number to listen on.
        state (Optional[str]): Current state of the task. 
        image (Optional[str]): Docker image name to use for the task.
        env_var (Optional[str]): Environment variable associated with the task.
        restart (Optional[bool]): Whether the task should restart on failure.
        outputs (Optional[List[str]]): List of output keys generated by the task.
        inputs (Optional[List[str]]): List of input keys required by the task.
        volume (Optional[Union[PersistentVolume, PersistentVolumeClaim]]): Volume to mount for remote storage.
    """
